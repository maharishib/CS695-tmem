...
int (* generic_glayer_hook) (int) = NULL;
EXPORT_SYMBOL(generic_glayer_hook);

static int handle_vmcall(struct kvm_vcpu *vcpu)
{	
	unsigned rax = kvm_register_read(vcpu, VCPU_REGS_RAX); 
	if(rax >= 100 && rax < 107 && generic_glayer_hook == NULL)
	{
		printk(KERN_INFO "Module not loaded\n");
		return 0;
	}
	else if (rax >= 100 && rax < 107)
		return generic_glayer_hook();
